    ...
    with conn.connection.cursor() as cur:
        if force:
            try:
                ...
                execute_logged(cur, sql_for_drop_table(name), conn.sql_tracer, conn)
            except dbapi.Error:
                pass
            except pyodbc.Error:
                pass
        # SQLTRACE added sql_tracer
        execute_logged(cur, sql_for_create_table(name, spec), conn.sql_tracer, conn)
        if data:
            # non-None, non-empty
    ...
    with conn.connection.cursor() as cur:
        ...
        if force:
            try:
                ...
                pass
        execute_logged(cur, statement, conn.sql_tracer, conn) # SQLTRACE added sql_tracer

def execute_logged(cursor, statement, sql_tracer=None, conn=None): # SQLTRACE added sql_tracer
    """
    ...
    if conn:
        if 'CALL' in statement.upper():
            conn.last_execute_statement = statement
    # SQLTRACE
    if sql_tracer:
        sql_tracer.trace_sql(statement)
    cursor.execute(statement)

def logged_without_execute(statement, sql_tracer=None, conn=None):
    """
    Logged info without execute sql.
    """
    logger.info("Executing SQL: %s", statement)
    if conn:
        conn.last_execute_statement = statement
