...
from hana_ml.visualizers.eda_plotly import distribution_plot, pie_plot, correlation_plot, scatter_plot, bar_plot, box_plot
        ...
        for feature in columns:
            if isinstance(bins, dict):
                if feature in bins:
                    ...
                    _bins = default_bins
            _, trace, _ = distribution_plot(data=data,
                                            column=feature,
                                            bins=_bins,
                                            debrief=debrief,
...
class EDAVisualizer(Visualizer):
    ...
    def __init__(self, ax=None, size=None, cmap=None, enable_plotly=False, fig=None):
        super(EDAVisualizer, self).__init__(ax=ax, size=size, cmap=cmap, enable_plotly=enable_plotly, fig=fig)
    ...
    def distribution_plot(self, data, column, bins, title=None, x_axis_fontsize=10, #pylint: disable= too-many-locals, too-many-arguments
                          ...
                          replacena=0, x_axis_label="", y_axis_label="", subplot_pos=(1, 1), **kwargs):
        """
        ...
        if self.enable_plotly:
            return distribution_plot(data, column, bins, title=title, x_axis_label=x_axis_label, y_axis_label=y_axis_label,
    ...
    def pie_plot(self, data, column, explode=0.03, title=None, legend=True,
        ...
        """
        if self.enable_plotly:
            return pie_plot(data, column, title=title, title_fontproperties=title_fontproperties, fig=self.fig,
                            subplot_pos=subplot_pos, **kwargs)
        else:
    ...
    def correlation_plot(self, data, key=None, corr_cols=None, label=True, cmap=None, title="Pearson's correlation (r)", **kwargs): #pylint: disable=too-many-locals
        ...
        if self.enable_plotly:
            if not cmap:
                cmap = "blues"
            return correlation_plot(data, key=key, corr_cols=corr_cols, cmap=cmap, title=title, **kwargs)
        else:
            if not cmap:
                cmap = "RdYlBu"
    ...
    def scatter_plot(self, data, x, y, x_bins=None, y_bins=None, title=None, label=None, #pylint: disable=too-many-locals, too-many-arguments, too-many-statements, invalid-name
        ...
        if self.enable_plotly:
            if not cmap:
                cmap = "blues"
            return scatter_plot(data, x, y, x_bins=x_bins, y_bins=y_bins, title=title,
    ...
    def bar_plot(self, data, column, aggregation, title=None, label_fontsize=12, title_fontproperties=None, orientation=None, **kwargs): #pylint: disable=too-many-branches, too-many-statements
        ...
        """
        if self.enable_plotly:
            return bar_plot(data, column, aggregation, title=title, orientation=orientation,
                            title_fontproperties=title_fontproperties, **kwargs)
        else:
...
class Profiler():
    ...
    def description(self, data, key, bins=20, missing_threshold=10, card_threshold=100, #pylint: disable=too-many-locals, too-many-arguments, too-many-branches, too-many-statements, no-self-use
        ...
        for i in numeric:
            skewness = cont.collect(geometries=False)['STAT_VALUE']
            stat = 'STAT_NAME'
            val = 'skewness'
            var = 'VARIABLE_NAME'
            skewness = skewness.loc[(cont.collect(geometries=False)[stat] == val) & (cont.collect(geometries=False)[var] == i)]
