            ...
            for class_1 in classes:
                for class_2 in classes:
                    ...
                    if confusion_matrix[cond].shape[0] == 0:
                        confusion_matrix = confusion_matrix.append({'ACTUAL_CLASS': class_1,
...
class GradientBoostingClassifier(_GradientBoostingClassifierBase):
    ...
    other_train_apl_aliases: dict, optional
        ...
        Please see *Common APL Aliases for Model Training* in the `SAP HANA APL Reference Guide
        <https://help.sap.com/viewer/p/apl>`_.
    ...
    has been exported in JSON format. See *JavaScript Runtime* in the `SAP HANA APL Reference
    ...
    def build_report(self):
        ...
        try:
            statistic_table = self.get_debrief_report('ClassificationRegression_Performance') \
                .filter(('("Partition" = \'Validation\''
                         'and "Indicator" != \'Prediction Confidence\')'
                         'or ("Partition" = \'Estimation\''
                         'and "Indicator" = \'Prediction Confidence\')')) \
                .select('Indicator', 'Value').rename_columns(['STAT_NAME', 'STAT_VALUE']).collect()
            statistic_table['CLASS_NAME'] = None
            metrics_per_class = self.get_metrics_per_class()
            statistic_per_class_table = []
            for stat_name, stat_values in metrics_per_class.items():
                for class_name, value in stat_values.items():
                    statistic_per_class_table.append({'STAT_NAME': stat_name,
                                                      'STAT_VALUE': float("{:.3f}".format(value)),
                                                      'CLASS_NAME': class_name})
            statistic_per_class_table = pd.DataFrame(statistic_per_class_table) \
                .replace('Precision', 'PRECISION') \
                .replace('Recall', 'RECALL') \
                .replace('F1Score', 'F1_SCORE')
            statistic_table = statistic_table.append(statistic_per_class_table)
            target_stats = self.get_debrief_report('MultiClassTarget_Statistics') \
                .filter('"Partition" = \'Validation\'').select('% Weight', 'Category') \
            ...
            statistic_table = statistic_table.append(target_stats)

            # pylint: disable=protected-access
            self._set_statistic_table(statistic_table) \
                ._set_metric_table(None) \
                ._render_report()
        except Exception as err:
            logger.error(str(err))
            raise
...
class GradientBoostingBinaryClassifier(_GradientBoostingClassifierBase):
    ...
    def get_performance_metrics(self):
        ...
        prediction_confidence = self.get_debrief_report('ClassificationRegression_Performance') \
            ...
            .collect()['Value'].iloc[0]
        ret.update({'PredictionConfidence': prediction_confidence})
        ret.update({'perf_per_iteration': self.get_evalmetrics()})
        ret.update({'BestIteration': self.get_best_iteration()})
        return ret
    ...
    def build_report(self):
        ...
        try:
            stat_table = self.get_debrief_report('ClassificationRegression_Performance') \
                .filter(('("Partition" = \'Validation\''
                         'and "Indicator" != \'Prediction Confidence\')'
                         'or ("Partition" = \'Estimation\''
                         'and "Indicator" = \'Prediction Confidence\')')) \
            ...
            target_stats = self.get_debrief_report('BinaryTarget_Statistics') \
                .filter('"Partition" = \'Validation\'').collect().iloc[0]
            ...
            stat_table = stat_table.append({'STAT_NAME': 'SUPPORT',
                                            'STAT_VALUE': support,
                                            'CLASS_NAME': target_key}, ignore_index=True)

            roc_curve = self.get_debrief_report('BinaryTarget_CurveRoc') \
                .select('False Positive Rate', 'True Positive Rate (Validation)') \
                .rename_columns(['ROC_FPR', 'ROC_TPR']).collect()
            metrics_table = pd.DataFrame(columns=['NAME', 'X', 'Y']) \
                .append(pd.DataFrame(({"NAME": name, "X": i, "Y": row[name]}
                                      for i, row in roc_curve.iterrows()
                                      for name in ['ROC_TPR', 'ROC_FPR'])),
                        ignore_index=True)
            cumlift_curves = self.get_debrief_report('BinaryTarget_CurveLift')
            cumlift_curves_names = {'Lift (Validation)': 'CUMLIFT',
                                    'Random': 'RANDOM_CUMLIFT',
                                    'Wizard': 'PERF_CUMLIFT'}
            for curve_name, curve_new_name in cumlift_curves_names.items():
                curve = cumlift_curves.select('% Population', curve_name) \
                    .rename_columns(['X', 'Y']).collect()
                curve['X'] = curve['X'].apply(lambda x: float(x) / 100.)
                curve.insert(loc=0, column='NAME', value=curve_new_name)
                metrics_table = metrics_table.append(curve)
            ...
            if int(version) >= 2123:
                cumgains_curves = self.get_debrief_report('BinaryTarget_CurveGain')
                cumgains_curves_names = {'Gain (Validation)': 'CUMGAINS',
                                         'Random': 'RANDOM_CUMGAINS',
                                         'Wizard': 'PERF_CUMGAINS'}
                for curve_name, curve_new_name in cumgains_curves_names.items():
                    curve = cumgains_curves.select('% Population', curve_name) \
                        .rename_columns(['X', 'Y']).collect()
                    curve['X'] = curve['X'].apply(lambda x: float(x) / 100.)
                    curve.insert(loc=0, column='NAME', value=curve_new_name)
                    metrics_table = metrics_table.append(curve)
            metrics_table = create_dataframe_from_pandas(self.conn_context, metrics_table,
                                                         '#METRICS_TABLE_{}'.format(self.id),
                                                         force=True, disable_progressbar=True)

            # pylint: disable=protected-access
            self._set_statistic_table(stat_table) \
                ._set_metric_table(metrics_table) \
                ._render_report()
        except Exception as err:
            logger.error(str(err))
            raise
