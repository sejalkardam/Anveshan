...
class AutoTimeSeries(APLBase): #pylint: disable=too-many-instance-attributes
    ...
    **{'APL/ApplyExtraMode': 'First Forecast with Stable Components and Residues and Error Bars'}**.
    ...
    in the `SAP HANA APL Reference Guide <https://help.sap.com/viewer/p/apl>`_.
    ...
    def get_model_components(self):
        ...
        Returns
        -------
        A dictionary with 3 possible keys: 'Trend', 'Cycles', 'Fluctuations'.
            ...
            for i in range(1, horizon + 1):
                ...
                for comp in components:
                    applyout_tbl_specs.extend([('kts_{}{}'.format(i, comp), 'DOUBLE'),
                                               ('kts_{}Residues{}'.format(i, comp), 'DOUBLE')])
    ...
    def _generate_debrief_tables(self):
        if self.model_table_ is None:
            raise FitIncompleteError("Model not initialized. Perform a fit first (fit_predict is not supported).")
        super()._generate_debrief_tables()
