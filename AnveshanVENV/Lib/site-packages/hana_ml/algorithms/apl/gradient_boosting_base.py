...
class GradientBoostingBase(APLBase):
    ...
    def _get_common_report_tables(self):
        ...
        for k in self.APL_ALIAS_KEYS:
            ...
            if param_val:
                param_table = param_table.append({'KEY': k, 'VALUE': param_val}, ignore_index=True)
        if getattr(self, 'other_train_apl_aliases', None):
            param_table = param_table.append({'KEY': 'other_train_apl_aliases',
                                              'VALUE': self.other_train_apl_aliases},
                                             ignore_index=True)
