    ...
    def _prep_init_param(self, conn, data, key):
        ...
        if self.n_components is not None:
            with conn.connection.cursor() as cur:
                execute_logged(cur,
                               'CREATE LOCAL TEMPORARY COLUMN TABLE {} ("ID" INTEGER, "CLUSTER_NUMBER" INTEGER);'.format(init_param_tbl),
                               conn.sql_tracer,
                               conn)
                execute_logged(cur,
                               'INSERT INTO {} VALUES (0, {});'.format(init_param_tbl, self.n_components),
                               conn.sql_tracer,
                               conn)
            ...
            with conn.connection.cursor() as cur:
                execute_logged(cur,
                               'CREATE LOCAL TEMPORARY COLUMN TABLE {} ("ID" INTEGER, "SEEDS" {});'.format(init_param_tbl, id_type),
                               conn.sql_tracer,
                               conn)
                for idx, val in enumerate(self.init_centers):
                    execute_logged(cur,
                                   'INSERT INTO {} VALUES ( {}, {} );'.format(init_param_tbl, idx, val),
                                   conn.sql_tracer,
                                   conn)
                init_param_data = conn.table(init_param_tbl)
