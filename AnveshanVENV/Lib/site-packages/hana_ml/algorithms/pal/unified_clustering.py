    ...
    def __init__(self,
        ...
        if self.func == 'GMM':
            if self.params.get('init_param') is None:
                err_msg = "'init_param' is a madantory parameter for gaussianmixture!"
                logger.error(err_msg)
                raise ValueError(err_msg)

            method = self.params['init_param']
            if method == 'manual' and self.params.get('init_centers') is None:
                err_msg = "'init_centers' is a madantory parameter when init_param is 'manual' of gaussianmixture!"
                logger.error(err_msg)
                raise ValueError(err_msg)
            if method != 'manual' and self.params.get('n_components') is None:
                err_msg = "'n_components' is a madantory parameter when init_param is not 'manual' of gaussianmixture!"
                logger.error(err_msg)
                raise ValueError(err_msg)
