...
class LinearRegression(PALBase):
    ...
    def score(self, data, key=None, features=None, label=None):
        ...
        float
            ...
            Returns the coefficient of determination R2 of the prediction.
        """
        if getattr(self, 'pmml_', None) is None and not hasattr(self, 'model_'):
            raise FitIncompleteError("Model not initialized. Perform a fit first.")
...
class OnlineLinearRegression(PALBase):
    r"""
    Online linear regression is an online version of the linear regression and is used when the training data are obtained multiple rounds.
    ...
    def partial_fit(self,
        ...
        if self.model_ is None:
            unique_id = str(uuid.uuid1()).replace('-', '_').upper()
            inter_tbl = '#PAL_LINEAR_REGRESSION_INTER_TBL_{}_{}'.format(self.id, unique_id)
        ...
        # create inter tbl based on model_ or intermediate_result_ (the first time)
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        inter_tbl_name = "#PAL_ONLINE_LINEAR_REGRESSION_INTER_TBL_{}_{}".format(self.id, unique_id)
        try:
            with conn.connection.cursor() as cur:
                execute_logged(cur,
                               'CREATE LOCAL TEMPORARY COLUMN TABLE {} ("SEQUENCE" INTEGER, "INTERMEDIATE_MODEL" NVARCHAR(5000))'.format(inter_tbl_name),
                               conn.sql_tracer,
                               conn)
                if self.model_ is not None:
                    execute_logged(cur,
                                   'INSERT INTO {} SELECT * FROM ({})'.format(inter_tbl_name, self.model_[1].select_statement),
                                   conn.sql_tracer,
                                   conn)
                else:
                    execute_logged(cur,
                                   'INSERT INTO {} SELECT * FROM ({})'.format(inter_tbl_name, self.intermediate_result_.select_statement),
                                   conn.sql_tracer,
                                   conn)
            ...
            logger.exception(str(db_err.args[1]))
        inter_tbl = conn.table(inter_tbl_name)
            ...
            if self.param_values is not None:
                for x in self.param_values:#pylint:disable=invalid-name
                    ...
                    if x[0] in set_param_list:
                        ...
                        logger.error(msg)
                        raise ValueError(msg)
                    if self.solver not in ((1, None) if self.multi_class else (-1, 0, 2, 3, 6, None)):
                        msg = ("Parameter `{}` is invalid when ".format(x[0])+
                               "`solver` is not set as "+
                               ("cyclical" if self.multi_class else "auto, newton, cyclical, lbfgs or proximal."))
                        logger.error(msg)
                        raise ValueError(msg)
                    if not (isinstance(x[1], list) and all(isinstance(t, (int, float)) for t in x[1])):#pylint:disable=line-too-long
                        msg = "Valid values of `{}` must be a list of numericals.".format(x[0])
            ...
            if self.param_range is not None:
                ...
                for x in self.param_range:#pylint:disable=invalid-name
                    ...
                    if x[0] in set_param_list:
                        ...
                        logger.error(msg)
                        raise ValueError(msg)
                    if self.solver not in ((1, None) if self.multi_class else (-1, 0, 2, 3, 6)):
                        msg = ("Parameter `{}` is invalid when ".format(x[0])+
                               "`solver` is not set as "+
                               ("cyclical." if self.multi_class else "auto, newton, cyclical, lbfgs or proximal."))
                        logger.error(msg)
...
class OnlineMultiLogisticRegression(PALBase):
    ...
    constant_values : str, optional

        Specifies the constant step size for each region defined by ``step_boundaries``.
    ...
    def partial_fit(self,
        ...
        if self.model_ is None:
            ...
            #print(self.online_result_.collect())
        ...
        # create inter tbl based on
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        input_tbl_name = "#PAL_ONLINE_MLR_INPUT_TBL_{}_{}".format(self.id, unique_id)
        try:
            with conn.connection.cursor() as cur:
                execute_logged(cur,
                               'CREATE LOCAL TEMPORARY COLUMN TABLE {} ("SEQUENCE" INTEGER, "SERIALIZED_RESULT" NVARCHAR(5000))'.format(input_tbl_name),
                               conn.sql_tracer,
                               conn)
                if self.model_ is not None:
                    execute_logged(cur,
                                   'INSERT INTO {} SELECT * FROM ({})'.format(input_tbl_name, self.model_[1].select_statement),
                                   conn.sql_tracer,
                                   conn)
                else:
                    execute_logged(cur,
                                   'INSERT INTO {} SELECT * FROM ({})'.format(input_tbl_name, self.online_result_.select_statement),
                                   conn.sql_tracer,
                                   conn)
            conn.connection.commit()
        except dbapi.Error as db_err:
            ...
            logger.exception(str(db_err.args[1]))
        input_tbl = conn.table(input_tbl_name)
