...
def outlier_detection_kmeans(data, # pylint: disable=too-many-locals
    ...
    require_pal_usable(conn)
    # SQLTRACE
    conn.sql_tracer.set_sql_trace(None, 'Clustering', 'anomaly_detection_kmeans')

    n_clusters = arg('n_clusters', n_clusters, int)
    distance_level = arg('distance_level', distance_level,
                         dict(manhattan=1, euclidean=2, minkowski=3))
    contamination = arg('contamination', contamination, float)
    sum_distance = arg('sum_distance', sum_distance, bool)
    init = arg('init', init,
               {'first_k':1, 'replace':2, 'no_replace':3, "patent":4})
    max_iter = arg('max_iter', max_iter, int)
    normalization = arg('normalization', normalization,
                        {'no':0, 'l1_norm':1, 'min_max':2})
    tol = arg('tol', tol, float)
    distance_threshold = arg('distance_threshold', distance_threshold, float)
    thread_number = arg('thread_number', thread_number, int)
    ...
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
    tables = ["#ANOMALY_DETECTION_KMEANS_{}_{}".format(name, unique_id)
              for name in ['OUTLIERS', 'STATS', 'CENTERS']]
    outliers_tbl, stats_tbl, centers_tbl = tables
    param_array = [('GROUP_NUMBER', n_clusters, None, None),
                   ('DISTANCE_LEVEL', distance_level, None, None),
                   ('OUTLIER_PERCENTAGE', None, contamination, None),
                   ('OUTLIER_DEFINE', int(sum_distance) + 1, None, None),
                   ('MAX_ITERATION', max_iter, None, None),
                   ('INIT_TYPE', init, None, None),
                   ('DISTANCE_LEVEL', distance_level, None, None),
                   ('NORMALIZATION', normalization, None, None),
                   ('EXIT_THRESHOLD', None, tol, None),
                   ('DISTANCE_THRESHOLD', None, distance_threshold, None),
                   ('THREAD_NUMBER', thread_number, None, None)]
    try:
        call_pal_auto_with_hint(conn,
                                None,
...
class SpectralClustering(PALBase):
    ...
    krylov_dim : int, optional
        ...
        .. Note::
            ...
            ``n_components`` < ``krylov_dim`` :math:`\le`
